...
COPY . /usr/src/reaction-parser
WORKDIR /usr/src/reaction-parser
RUN mvn -Pdocker clean package

FROM openjdk:8-jre
# VOLUME /logs
RUN mkdir /temps
ENV SPRING_BOOT_APP reaction-parser.jar
...
COPY --from=builder /usr/src/reaction-parser/target/$SPRING_BOOT_APP ./
EXPOSE 8083 
